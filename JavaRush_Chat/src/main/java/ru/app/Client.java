...
class ClientSomthing {
  ...
  public ClientSomthing(String addr, int port) {
    ...
    try {
      ...
      new ReadMsg()
          .start(); // нить читающая сообщения из сокета в бесконечном цикле
      new WriteMsg().start(); // нить пишущая сообщения в сокет приходящие с
                              // консоли в бесконечном цикле
    } catch (IOException e) {
    ...
    public void run() {
      while (true) {
        ...
        try {
          ...
          dt1 =
              new SimpleDateFormat("HH:mm:ss"); // берем только время до секунд
...
public class Client {
  ...
  public static void main(String[] args) { new ClientSomthing(ipAddr, port); }
}
